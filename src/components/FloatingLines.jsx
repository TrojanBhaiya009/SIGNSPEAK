...
float wave(vec2 uv, float offset, vec2 screenUv, vec2 mouseUv, bool shouldBend) {
  ...
  float x_movement = time * 0.4;
  float amp        = 0.3 + sin(offset * 1.3) * 0.05;
  float y          = sin(uv.x * 0.8 + x_offset + x_movement) * amp
                   + sin(uv.x * 1.6 + x_offset * 0.6 + x_movement * 0.7) * amp * 0.25
                   + sin(uv.x * 0.4 + x_offset * 1.5 + x_movement * 1.2) * amp * 0.15;
  ...
  float m = uv.y - y;
  return smoothstep(0.055, 0.0, abs(m)) * 0.85;
}
...
export default function FloatingLines({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (ro && containerRef.current) { // eslint-disable-line react-hooks/exhaustive-deps
        ro.disconnect();
      }
      if (interactive) {
